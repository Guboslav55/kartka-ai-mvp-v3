...
async function buildDallePrompt(
  ...
  // Use GPT-4o to analyze photo and suggest better prompt
  const analysis = await openai.chat.completions.create({
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: `Product: "${productName}". Category features: ${bulletText}. Create a DALL-E 3 background prompt for ${variant} infographic style. ${variantMap[variant]} Return only the prompt, max 200 words.` },
      ],
    }],
    max_tokens: 200,
  });
...
async function generateBackground(prompt: string): Promise<Buffer | null> {
  try {
    const res = await openai.images.generate({
      model: 'dall-e-3',
      prompt: `${prompt} IMPORTANT: NO text, NO letters, NO words, NO labels anywhere in the image.`,
      size: '1024x1024',
      quality: 'hd',
      style: 'natural',
      n: 1,
...
async function compositeWithProduct(
  bgBuf: Buffer,
  ...
  // Resize product to fit nicely in center (50-60% of canvas)
  ...
  // Position product center-bottom area
  ...
  <!-- Gradient overlays -->
  <rect width="1024" height="160" fill="url(#topGrad)"/>
  <rect y="864" width="1024" height="160" fill="url(#botGrad)"/>
  <!-- Accent bar top -->
  ...
  <!-- Benefits bottom -->
  ${topBullets.map((b, i) => `
  <rect x="20" y="${880 + i * 42}" width="${Math.min(b.length * 13 + 50, 700)}" height="36" rx="6" fill="${textBg}"/>
  <text x="50" y="${904 + i * 42}" font-family="Arial,sans-serif" font-size="20" fill="${textColor}">✓ ${escapeXml(b)}</text>`).join('')}
  ...
  return sharp(bgBuf)
    .composite([
      { input: productResized, top, left, blend: 'over' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Composite product + text overlay
