  ...
  try {
    ...
    // ── PHOTO MODE ────────────────────────────────────────────────────────────
      ...
      const STYLES: Record<string, string> = {
        ...
        store:    'This EXACT product presented on a clean modern retail display stand or shelf in a stylish boutique. Product remains 100% identical. Show the SAME side as in the reference: a BACK view stays a BACK view — never rotate or flip it to the front, and never fabricate front-only details (chest logo/wordmark, front pocket, drawstrings) absent from the reference. Tasteful staging, soft retail lighting, shallow depth of field.',
        flatlay:  'Completely RESTAGE this EXACT product as a strict top-down FLAT-LAY: remove the hanger, remove the original background, wall, poster or painting entirely, and lay the garment FLAT on a clean styled surface (neutral fabric, wood or stone). Photograph it straight from directly above at a true 90-degree overhead angle, the garment spread out flat, fully visible, neatly arranged, zipped and symmetric. Show the SAME side of the garment as in the reference — if the reference shows the BACK, lay the BACK flat and keep it as a back view; never flip a back view into a front. Keep the EXACT same design: a full-zip jacket stays a full-zip jacket (do NOT turn it into a pullover), do NOT add a kangaroo or front pouch pocket, hood drawstrings, or any pocket, stripe or feature that is not in the reference. Do NOT show it hanging or on a hanger and do NOT keep the original backdrop. The product itself stays 100% identical in colour, fabric, logos and details. Even soft daylight, subtle realistic shadows.',
        catalog:  'This EXACT product alone, perfectly centred on a seamless studio background with a soft gradient and a subtle ground shadow, clean even e-commerce lighting. Product remains 100% identical. Show the SAME side of the product as in the reference: if the reference is a BACK view, keep the result a BACK view — never rotate, mirror or flip the garment to reveal the front, and never invent front-only details (chest NIKE wordmark, front kangaroo pocket, hood drawstrings, zipper pull) that are not visible in the reference. Keep it on a hanger only if the reference is on a hanger; otherwise present it cleanly without inventing a new orientation.',
      ...
      if (displayStyle === 'catalog' && !REPLICATE) {
        ...
        for (let i = 0; i < usableUrls.length; i++) {
          try {
            ...
            let prompt = base
            prompt += ' CRITICAL: keep the product 100% identical to the reference photo — the SAME colour (do NOT recolour, tint, lighten, darken or apply any pastel/warm/cool colour cast to the product itself; if the reference garment is black it must stay black), the same shape, fabric, proportions and design. Reproduce every brand logo, Nike swoosh and printed wordmark EXACTLY — sharp, undistorted and clearly legible, with the "NIKE" lettering spelled and shaped correctly, never blurred, warped or turned into random characters. Do NOT invent or add features: no hood drawstrings, cords, laces or toggles, no extra zippers, buttons, pockets, straps or seams that are not visibly present in the reference, and do not remove any zipper or closure that is present. If the hood has no drawstring, keep it without one. Keep the SAME side and orientation as the reference: if the reference shows the BACK of the garment, the result MUST also show the back — never rotate, mirror or flip it to reveal the front, and never fabricate front-only details (chest logo or NIKE wordmark, front kangaroo/pouch pocket, hood drawstrings, zipper pull) that are not visible in the reference photo. Preserve the EXACT geometry of every stripe, panel line and seam: an angled or chevron (V-shaped) stripe must keep its exact angle and shape and must NEVER be straightened into a flat horizontal band or moved to another position. Preserve the closure type and pockets exactly: if the reference is a FULL-ZIP jacket, keep the full-length front zipper and the zip pockets — never convert it into a pullover and never remove the zipper or pockets; if the reference is a pullover, keep it a pullover.'
