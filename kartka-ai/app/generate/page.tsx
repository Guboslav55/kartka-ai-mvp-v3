...
export default function GeneratePage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase.from('users').select('cards_left, plan').eq('id', user.id).single()
        .then(({ data }) => {
          if (data) { setCardsLeft(data.cards_left); setUserPlan(data.plan); }
          setReady(true);
        });
    });
  }, []);
  ...
  const generate = useCallback(async () => {
    ...
    setLoading(false);
  }, [productName, category, features, platform, tone, lang, genImage, cardsLeft, loading]);
    ...
    <div className="min-h-screen px-4 sm:px-6 py-8 max-w-3xl mx-auto">
      ...
      <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-5 sm:p-7 mb-5">
        <div className="space-y-5">
          ...
          {/* Product name */}
          <div>
            <label className="block text-gold text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-2">
              Назва товару *
            </label>
            <input
              value={productName}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)} disabled={noCards}
                ...
                {['Електроніка','Одяг та взуття','Дім та сад','Краса та здоров\'я','Спорт та хобі','Авто та мото','Іграшки','Інше'].map(c => (
          ...
          <div>
            ...
            <textarea value={features} onChange={e => setFeatures(e.target.value)} rows={3} disabled={noCards}
              placeholder="наприклад: шумозаглушення 30 дБ, 30 год заряду, Bluetooth 5.2, колір чорний"
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <button type="button" onClick={() => setGenImage(v => !v)} disabled={noCards}
              ...
              <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${genImage ? 'left-[18px]' : 'left-0.5'}`} />
            </button>
      ...
      {/* Skeleton */}
      {loading && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 animate-fade-up">
      ...
      {/* Result */}
      {result && !loading && (
        <div className="bg-white rounded-2xl overflow-hidden shadow-2xl animate-fade-up">
          ...
          <div className="p-5 sm:p-7">
            {result.imageUrl && (
              <img src={result.imageUrl} alt={result.title}
                className="w-full h-40 sm:h-52 object-cover rounded-xl mb-6" />
            )}
            ...
            <div className="flex flex-wrap gap-2">
              {result.keywords.map(k => (
                <span key={k} className="bg-blue-50 text-navy text-xs font-medium px-3 py-1 rounded-full">{k}</span>
              ))}
            </div>
          </div>
          ...
          <div className="px-5 sm:px-7 pb-6 grid grid-cols-1 sm:grid-cols-3 gap-2">
            <button onClick={copyAll}
              className="bg-gray-900 text-white px-4 py-3 rounded-xl text-sm font-semibold hover:bg-gray-700 transition-colors flex items-center justify-center gap-2">
