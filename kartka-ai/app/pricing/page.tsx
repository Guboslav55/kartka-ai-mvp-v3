...
// ──────────────────────────────────────────────
// LiqPay форма (відправляє на liqpay.ua)
// ──────────────────────────────────────────────
function LiqPayForm({ data, signature }: { data: string; signature: string }) {
  return (
    <form method="POST" action="https://www.liqpay.ua/api/3/checkout" acceptCharset="utf-8">
      ...
      <button
        type="submit"
        className="w-full rounded-xl bg-indigo-600 py-3 text-white font-semibold hover:bg-indigo-500 transition-colors text-lg"
      >
        Перейти до оплати →
      </button>
    </form>
...
function PurchaseModal({
  ...
  pkg: (typeof STAR_PACKAGES)[number]
  onClose: () => void
}) {
  ...
  async function applyPromo() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setPromoApplied(d)
        // Якщо дав зорі — оновити баланс
        if (d.type === 'stars') {
          window.dispatchEvent(new CustomEvent('stars-updated', { detail: { newBalance: 0 } }))
        }
      }
    } catch {
  ...
  async function initPayment() {
    ...
    try {
      const res = await fetch('/api/payment/create', {
        ...
        body: JSON.stringify({
          packageId: pkg.id,
          promoCode: promoApplied?.type === 'discount' ? promoCode : undefined,
        }),
      })
      const d = await res.json()
      if (!res.ok) {
        ...
        setError(d.error || 'Помилка ініціації платежу')
        return
      }
      setPaymentData(d)
    } catch {
      ...
      <div className="w-full max-w-md rounded-2xl bg-[#1A1A2E] border border-white/10 p-6 shadow-2xl">
        ...
        <div className="mb-5 rounded-xl bg-white/5 p-4">
          <div className="flex justify-between text-sm mb-2">
            <span className="text-gray-400">Зорі</span>
            <span className="text-white font-medium">⭐ {pkg.stars.toLocaleString('uk-UA')}</span>
          </div>
          {pkg.bonus_stars > 0 && (
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-400">Бонус</span>
              <span className="text-green-400 font-medium">+ ⭐ {pkg.bonus_stars.toLocaleString('uk-UA')}</span>
            </div>
          )}
          <div className="border-t border-white/10 pt-2 mt-2 flex justify-between">
            <span className="text-gray-300 font-medium">Разом</span>
            <span className="text-white font-bold">⭐ {total.toLocaleString('uk-UA')}</span>
          </div>
        </div>
        ...
        {!paymentData && (
          <div className="mb-5">
            ...
            {promoApplied ? (
              <div className="rounded-xl bg-green-500/10 border border-green-500/30 p-3 text-sm text-green-400">
        ...
        {!paymentData && (
          <div className="mb-5 flex items-center justify-between">
            ...
            <div className="text-right">
              {discountPct > 0 && (
                <div className="text-sm text-gray-500 line-through">{pkg.price_uah} грн</div>
              )}
              <div className="text-2xl font-bold text-white">{finalPrice} грн</div>
              {discountPct > 0 && (
                <div className="text-sm text-green-400">-{discountPct}%</div>
              )}
            </div>
          </div>
        )}
        ...
        {paymentData ? (
          ...
          >
            {loading ? (
              ...
              `Оплатити ${finalPrice} грн →`
            )}
          </button>
        )}

        <p className="mt-4 text-center text-xs text-gray-500">
          Оплата через LiqPay · Visa/Mastercard UAH · Зорі надходять миттєво
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div
      className={`
        ...
        ${pkg.is_popular
          ? 'bg-indigo-600/20 border-indigo-500 shadow-indigo-500/20 shadow-lg'
    ...
    >
      ...
      <div className="mb-4">
        <div className="text-3xl font-bold text-white">
          ⭐ {pkg.stars.toLocaleString('uk-UA')}
        </div>
        {pkg.bonus_stars > 0 && (
          <div className="text-sm text-green-400 font-medium mt-1">
            + {pkg.bonus_stars.toLocaleString('uk-UA')} бонусних зорь
          </div>
        )}
        {pkg.bonus_stars > 0 && (
          <div className="text-xs text-gray-400 mt-0.5">
            Всього: {total.toLocaleString('uk-UA')} зорь
          </div>
        )}
      </div>
      ...
      <div className="mt-auto">
        <div className="flex items-end justify-between mb-3">
          ...
          <div className="text-sm text-gray-400">{perStar} грн/зоря</div>
        </div>
        <button
          className={`
...
function CostTable() {
  const items = [
    ...
    { icon: '🔍', name: 'Аналіз товару (авто-визначення)', cost: 0 },
  ]
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-white/5">
            <th className="px-4 py-3 text-left text-sm text-gray-400 font-medium">Операція</th>
            <th className="px-4 py-3 text-right text-sm text-gray-400 font-medium">Вартість</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, i) => (
      ...
      <main className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="text-lg text-gray-400 max-w-xl mx-auto">
            Зорі — внутрішня валюта КарткаАІ. Не згорають, без обмежень терміну.
          </p>
        ...
        <div className="mb-16 max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              {
                q: 'Що таке Зорі?',
                a: 'Зорі — внутрішня валюта КарткаАІ. Кожна операція (генерація фото, відео, інфографіки) витрачає певну кількість зорь.',
              },
              {
                q: 'Чи згорають Зорі?',
                a: 'Ні. Зорі на вашому балансі діють безстроково — використовуйте коли зручно.',
              },
              {
                q: 'Коли зарахуються зорі після оплати?',
                a: 'Миттєво після підтвердження платежу LiqPay. Зазвичай до 1 хвилини.',
              },
              {
                q: 'Що якщо генерація не вдалась?',
                a: 'Зорі повертаються автоматично. Якщо не повернулись — напишіть нам у Telegram.',
              },
              {
                q: 'Чи є безкоштовний план?',
                a: 'Так! При реєстрації ви отримуєте 5 безкоштовних зорь та 3 безкоштовні регенерації.',
              },
              {
                q: 'Можна отримати повернення коштів?',
                a: 'Так, протягом 7 днів без питань. Напишіть нам в Telegram-підтримку.',
              },
            ].map((item, i) => (
        ...
        <div className="text-center rounded-2xl bg-indigo-600/20 border border-indigo-500/30 p-8">
          ...
          <Link
            href="/register"
            className="inline-block rounded-xl bg-indigo-600 px-8 py-3 text-white font-semibold hover:bg-indigo-500 transition-colors"
          >
